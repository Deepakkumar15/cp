...
public:
    ...
    void apply(node &x, ll tl, ll tr){
        idx==2 ? x.v = (tr-tl+1) * v : x.v += (tr-tl+1) * v ;
    }
};
...
signed main(){
    ...
    while(q--){
        ...
        cin >> x ;
        if(x==1){
            // INCREASE EACH VALUE IN RANGE BY X (IDX==1)
            ll l, r, val ;
            cin >> l >> r >> val ;
            s.modify(l-1, r-1, val, 1) ;
        }
        
        else if(x==2){
            // SET EACH VALUE IN RANGE TO X (IDX==2)
            ll l, r, val ;
            cin >> l >> r >> val ;
            s.modify(l-1, r-1, val, 2) ;
        }
